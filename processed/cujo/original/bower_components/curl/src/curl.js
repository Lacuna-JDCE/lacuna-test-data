 ...
 *
 */
(function (global) {___jdce_logger("/bower_components/curl/src/curl.js", 0);
//"use strict"; don't restore this until the config routine is refactored
	...
	function noop () {___jdce_logger("/bower_components/curl/src/curl.js", 1);}

	function isType (obj, type) {___jdce_logger("/bower_components/curl/src/curl.js", 2);
		return toString.call(obj).indexOf('[object ' + type) == 0;
	}

	function normalizePkgDescriptor (descriptor) {___jdce_logger("/bower_components/curl/src/curl.js", 3);
		var main;
	...
	function isRelUrl (it) {___jdce_logger("/bower_components/curl/src/curl.js", 4);
		return it.charAt(0) == '.';
	}

	function isAbsUrl (it) {___jdce_logger("/bower_components/curl/src/curl.js", 5);
		return absUrlRx.test(it);
	}

	function joinPath (path, file) {___jdce_logger("/bower_components/curl/src/curl.js", 6);
		return removeEndSlash(path) + '/' + file;
	}

	function removeEndSlash (path) {___jdce_logger("/bower_components/curl/src/curl.js", 7);
		return path && path.charAt(path.length - 1) == '/' ? path.substr(0, path.length - 1) : path;
	}

	function reduceLeadingDots (childId, baseId) {___jdce_logger("/bower_components/curl/src/curl.js", 8);
		...
		if (isRelUrl(normId)) {
			isRelative = true;
			normId = normId.replace(findDotsRx, function (m, dot, doubleDot, remainder) {___jdce_logger("/bower_components/curl/src/curl.js", 9);
	...
	function pluginParts (id) {___jdce_logger("/bower_components/curl/src/curl.js", 10);
	...
	function Begetter () {___jdce_logger("/bower_components/curl/src/curl.js", 11);}

	function beget (parent, mixin) {___jdce_logger("/bower_components/curl/src/curl.js", 12);
	...
	function Promise () {___jdce_logger("/bower_components/curl/src/curl.js", 13);
		...
		function then (resolved, rejected, progressed) {___jdce_logger("/bower_components/curl/src/curl.js", 14);
			// capture calls to callbacks
			thens.push([resolved, rejected, progressed]);
		}

		function notify (which, arg) {___jdce_logger("/bower_components/curl/src/curl.js", 15);
		...
		complete = function promiseComplete (success, arg) {___jdce_logger("/bower_components/curl/src/curl.js", 16);
			// switch over to sync then()
			then = success ?
				function (resolved, rejected) {___jdce_logger("/bower_components/curl/src/curl.js", 17); resolved && resolved(arg); } :
				function (resolved, rejected) {___jdce_logger("/bower_components/curl/src/curl.js", 18); rejected && rejected(arg); };
		...
		this.then = function (resolved, rejected, progressed) {___jdce_logger("/bower_components/curl/src/curl.js", 19);
			then(resolved, rejected, progressed);
			return self;
		};
		this.resolve = function (val) {___jdce_logger("/bower_components/curl/src/curl.js", 20);
			self.resolved = val;
			complete(true, val);
		};
		this.reject = function (ex) {___jdce_logger("/bower_components/curl/src/curl.js", 21);
			self.rejected = ex;
			complete(false, ex);
		};
		this.progress = function (msg) {___jdce_logger("/bower_components/curl/src/curl.js", 22);
			notify(2, msg);
		}

	}

	function isPromise (o) {___jdce_logger("/bower_components/curl/src/curl.js", 23);
		return o instanceof Promise || o instanceof CurlApi;
	}

	function when (promiseOrValue, callback, errback, progback) {___jdce_logger("/bower_components/curl/src/curl.js", 24);
	 ...
	 * @returns {Function}
	 */
	function countdown (howMany, lambda, completed) {___jdce_logger("/bower_components/curl/src/curl.js", 25);
		var result;
		return function () {___jdce_logger("/bower_components/curl/src/curl.js", 26);
	...
	core = {
		 ...
		 * @return {*}
		 */
		toAbsId: function (id, parentId, cfg) {___jdce_logger("/bower_components/curl/src/curl.js", 27);
		...
		createContext: function (cfg, baseId, depNames, isPreload) {___jdce_logger("/bower_components/curl/src/curl.js", 28);
			...
			function toAbsId (childId) {___jdce_logger("/bower_components/curl/src/curl.js", 29);
				return core.toAbsId(childId, def.id, cfg);
			}

			function toUrl (n) {___jdce_logger("/bower_components/curl/src/curl.js", 30);
			...
			function localRequire (ids, callback, errback) {___jdce_logger("/bower_components/curl/src/curl.js", 31);
				var cb, rvid, childDef, earlyExport;

				// this is public, so send pure function
				// also fixes issue #41
				cb = callback && function () {___jdce_logger("/bower_components/curl/src/curl.js", 32); callback.apply(undef, arguments[0]); };
		...
		createResourceDef: function (cfg, id, isPreload) {___jdce_logger("/bower_components/curl/src/curl.js", 33);
			...
			execute = countdown(1, function (deps) {___jdce_logger("/bower_components/curl/src/curl.js", 34);
				def.deps = deps;
				try {
			...
			// before resolving
			def.resolve = function resolve (deps) {___jdce_logger("/bower_components/curl/src/curl.js", 35);
				when(isPreload || preload, function () {___jdce_logger("/bower_components/curl/src/curl.js", 36);
					origResolve((cache[def.id] = urlCache[def.url] = execute(deps)));
				});
			};

			// track exports
			def.exportsReady = function executeFactory (deps) {___jdce_logger("/bower_components/curl/src/curl.js", 37);
				when(isPreload || preload, function () {___jdce_logger("/bower_components/curl/src/curl.js", 38);
		...
		createPluginDef: function (cfg, id, resId, isPreload) {___jdce_logger("/bower_components/curl/src/curl.js", 39);
			var def;
		...
		getCjsRequire: function (def) {___jdce_logger("/bower_components/curl/src/curl.js", 40);
			return def.require;
		},

		getCjsExports: function (def) {___jdce_logger("/bower_components/curl/src/curl.js", 41);
			return def.exports || (def.exports = {});
		},

		getCjsModule: function (def) {___jdce_logger("/bower_components/curl/src/curl.js", 42);
			...
			if (!module) {
				module = def.module = {
					...
					'config': function () {___jdce_logger("/bower_components/curl/src/curl.js", 43); return def.config; }
				};
		...
		getDefUrl: function (def) {___jdce_logger("/bower_components/curl/src/curl.js", 44);
		 ...
		 */
		setApi: function (cfg) {___jdce_logger("/bower_components/curl/src/curl.js", 45);
			/*
			scenarios:
		...
		config: function (cfg) {___jdce_logger("/bower_components/curl/src/curl.js", 46);
			...
			function fixAndPushPaths (coll, isPkg) {___jdce_logger("/bower_components/curl/src/curl.js", 47);
			...
			function convertPathMatcher (cfg) {___jdce_logger("/bower_components/curl/src/curl.js", 48);
				var pathMap = cfg.pathMap;
				cfg.pathRx = new RegExp('^(' +
					cfg.pathList.sort(function (a, b) {___jdce_logger("/bower_components/curl/src/curl.js", 49); return pathMap[b].specificity - pathMap[a].specificity; } )
						.join('|')
						.replace(/\/|\./g, '\\$&') +
					')(?=\\/|$)'
				);
		...
		resolvePathInfo: function (absId, cfg) {___jdce_logger("/bower_components/curl/src/curl.js", 50);
			...
			if (!isAbsUrl(absId)) {
				path = absId.replace(cfg.pathRx, function (match) {___jdce_logger("/bower_components/curl/src/curl.js", 51);
		...
		resolveUrl: function (path, cfg) {___jdce_logger("/bower_components/curl/src/curl.js", 52);
			var baseUrl = cfg.baseUrl;
			return baseUrl && !isAbsUrl(path) ? joinPath(baseUrl, path) : path;
		},

		checkToAddJsExt: function (url, cfg) {___jdce_logger("/bower_components/curl/src/curl.js", 53);
		...
		loadScript: function (def, success, failure) {___jdce_logger("/bower_components/curl/src/curl.js", 54);
			...
			function process (ev) {___jdce_logger("/bower_components/curl/src/curl.js", 55);
				ev = ev || global.event;
			...
			function fail (e) {___jdce_logger("/bower_components/curl/src/curl.js", 56);
		...
		extractCjsDeps: function (defFunc) {___jdce_logger("/bower_components/curl/src/curl.js", 57);
			...
			source.replace(removeCommentsRx, '').replace(findRValueRequiresRx, function (m, rq, id, qq) {___jdce_logger("/bower_components/curl/src/curl.js", 58);
		...
		fixArgs: function (args) {___jdce_logger("/bower_components/curl/src/curl.js", 59);
			// resolve args
			...
			return {
				id: id,
				deps: deps || [],
				res: defFuncArity >= 0 ? defFunc : function () {___jdce_logger("/bower_components/curl/src/curl.js", 60); return defFunc; },
				cjs: cjs
			};
		},

		executeDefFunc: function (def) {___jdce_logger("/bower_components/curl/src/curl.js", 61);
		...
		defineResource: function (def, args) {___jdce_logger("/bower_components/curl/src/curl.js", 62);
		...
		getDeps: function (parentDef) {___jdce_logger("/bower_components/curl/src/curl.js", 63);
			...
			function collect (dep, index, alsoExport) {___jdce_logger("/bower_components/curl/src/curl.js", 64);
				deps[index] = dep;
			...
			function getDep (name, index) {___jdce_logger("/bower_components/curl/src/curl.js", 65);
				var resolveOnce, exportOnce, childDef, earlyExport;

				resolveOnce = countdown(1, function (dep) {___jdce_logger("/bower_components/curl/src/curl.js", 66);
					exportOnce(dep);
					resolveCollector(dep, index);
				});
				exportOnce = countdown(1, function (dep) {___jdce_logger("/bower_components/curl/src/curl.js", 67);
				...
				when(childDef,
					resolveOnce,
					parentDef.reject,
					parentDef.exports && function (msg) {___jdce_logger("/bower_components/curl/src/curl.js", 68);
			...
			function allResolved () {___jdce_logger("/bower_components/curl/src/curl.js", 69);
				parentDef.resolve(deps);
			}

			function allExportsReady () {___jdce_logger("/bower_components/curl/src/curl.js", 70);
				parentDef.exportsReady && parentDef.exportsReady(deps);
			}

		},

		fetchResDef: function (def) {___jdce_logger("/bower_components/curl/src/curl.js", 71);
			...
			core.loadScript(def,
				...
				function () {___jdce_logger("/bower_components/curl/src/curl.js", 72);
					var args = argsNet;
		...
		fetchDep: function (depName, parentDef) {___jdce_logger("/bower_components/curl/src/curl.js", 73);
			...
			if (mainId == loaderId) {
				...
				when(def, function(plugin) {___jdce_logger("/bower_components/curl/src/curl.js", 74);
					...
					if (!(fullId in cache)) {
						...
						var loaded = function (res) {___jdce_logger("/bower_components/curl/src/curl.js", 75);
		...
		getCurrentDefName: function () {___jdce_logger("/bower_components/curl/src/curl.js", 76);
		...
		findScript: function (predicate) {___jdce_logger("/bower_components/curl/src/curl.js", 77);
			var i = 0, script;
		...
		extractDataAttrConfig: function (cfg) {___jdce_logger("/bower_components/curl/src/curl.js", 78);
			var script;
			script = core.findScript(function (script) {___jdce_logger("/bower_components/curl/src/curl.js", 79);
				var main;
		...
		nextTurn: function (task) {___jdce_logger("/bower_components/curl/src/curl.js", 80);
			setTimeout(task, 0);
		}
	...
	function _curl (/* various */) {___jdce_logger("/bower_components/curl/src/curl.js", 81);
	...
	function _config (cfg, callback, errback) {___jdce_logger("/bower_components/curl/src/curl.js", 82);
		...
		if (cfg) {
			...
			if ('preloads' in cfg) {
				...
				core.nextTurn(function () {___jdce_logger("/bower_components/curl/src/curl.js", 83); preload = pPromise; });
			}
			...
			if (main) {
				...
				fallback = main[1]
					? function () {___jdce_logger("/bower_components/curl/src/curl.js", 84); new CurlApi(main[1], mPromise.resolve, mPromise.reject); }
					: mPromise.reject;
	...
	function CurlApi (ids, callback, errback, waitFor, isPreload) {___jdce_logger("/bower_components/curl/src/curl.js", 85);
		var then, ctx;

		ctx = core.createContext(userCfg, undef, [].concat(ids), isPreload);

		this['then'] = this.then = then = function (resolved, rejected) {___jdce_logger("/bower_components/curl/src/curl.js", 86);
			when(ctx,
				// return the dependencies as arguments, not an array
				function (deps) {___jdce_logger("/bower_components/curl/src/curl.js", 87);
					if (resolved) resolved.apply(undef, deps);
				},
				// just throw if the dev didn't specify an error handler
				function (ex) {___jdce_logger("/bower_components/curl/src/curl.js", 88);
		...
		this['next'] = function (ids, cb, eb) {___jdce_logger("/bower_components/curl/src/curl.js", 89);
			// chain api
		...
		core.nextTurn(function () {___jdce_logger("/bower_components/curl/src/curl.js", 90);
			when(isPreload || preload, function () {___jdce_logger("/bower_components/curl/src/curl.js", 91);
				when(waitFor, function () {___jdce_logger("/bower_components/curl/src/curl.js", 92); core.getDeps(ctx); }, errback);
			});
		});
	}
	...
	function _define (args) {___jdce_logger("/bower_components/curl/src/curl.js", 93);
	...
	function define () {___jdce_logger("/bower_components/curl/src/curl.js", 94);
	...
	cache['curl/_privileged'] = {
		'core': core,
		'cache': cache,
		'config': function () {___jdce_logger("/bower_components/curl/src/curl.js", 95); return userCfg; },
		'_define': _define,
		'_curl': _curl,
		'Promise': Promise
	};
